...
class LabjackLogger( ThreadedLogger ):
    ...
    def __init__(self, config_file, json_logger,in_queue=None):
        ...
        if self.streaming or self.analog or self.digital:
            try:
                self.labjack = u6.U6()
                self.log( 'Labjack opened.', 'info')
                self.labjack_active = True
                for i in range(4):
                    self.labjack.writeRegister(6000+i,0) # set labjack low
            except Exception, e:
                print "---------------------------------------------"
                print " Labjack failed to open due to an error"
                print " of type", e
                print "---------------------------------------------"
                self.labjack_active = False
        else:
            print "---------------------------------------------"
            print "           Labjack not initialized.          "
            print "---------------------------------------------"
            self.labjack_active = False
            
        if self.streaming and self.labjack_active:
            print "\t---> Streaming addresses:", self._streaming_addresses
            self.log('Streaming addresses:' + str(self._streaming_addresses), 'info')
            self._initialize_streaming()
        if self.analog and self.labjack_active:
        ...
        if self.digital and self.labjack_active:
