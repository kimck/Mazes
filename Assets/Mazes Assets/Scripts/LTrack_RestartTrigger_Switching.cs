...
public class LTrack_RestartTrigger_Switching : MonoBehaviour {
	...
	void Start () {
 	...
 	void OnTriggerEnter (Collider col) {	
    	LTrack_RewardTrigger_Switching.outcome = "'restart'";
	}
...
//		}
		...
		if (currentrun == 1){
			nextmaze=run1[LTrack_RewardTrigger_Switching.all_trial_num];
		}
		else if (currentrun == 2){
			nextmaze=run2[LTrack_RewardTrigger_Switching.all_trial_num];
		}
		else if (currentrun == 3){
			nextmaze=run3[LTrack_RewardTrigger_Switching.all_trial_num];
		}
